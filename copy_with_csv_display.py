...
def main():
    ...
    try:
        ...
        MapsBridge.LogInfo(f"Generated and saved random CSV to: {csv_output_path}")
        
        # ✅ DISPLAY CSV CONTENT IN CONSOLE OUTPUT
        MapsBridge.LogInfo("=" * 60)
        MapsBridge.LogInfo("CSV DATA OUTPUT:")
        MapsBridge.LogInfo("=" * 60)
        ...
        # Continue processing even if CSV fails
    
    # --- End New CSV Output Section ---
    ...
    # Store the CSV file as an output (for download)
    try:
        MapsBridge.StoreFile(
            csv_output_path,
            overwrite=True,
            keepFile=True,
            targetTileSetGuid=outputTileSetInfo.TileSet.Guid
        )
        MapsBridge.LogInfo(f"Stored CSV file for download: {csv_output_path}")
    except Exception as e:
        MapsBridge.LogError(f"Error storing CSV file: {e}")

    MapsBridge.AppendNotes("Image copied without modification. Random CSV data generated and displayed.")
